...
import { PLACES } from '../src/proto/shared/content.ts'
...
type Group = 'career' | 'projects' | 'galleries' | 'blog' | 'site' | 'prose'
const GROUPS: Group[] = ['career', 'projects', 'galleries', 'blog', 'site', 'prose']
...
function collectImageRefs(): ImageRef[] {
  ...
  // the four travel photos of the nomad section
  for (const [i, p] of PLACES.entries()) add(p.image, `content.ts:PLACES[${i}].image`, 'site')

  // the images inside the blog and project bodies (CMS HTML): the S3 buckets
  // refuse every origin but production, so they must be served from here
  const inline = (html: string, ref: string) => {
    for (const m of html.matchAll(/<img\b[^>]*\bsrc=\\?["']([^"'\\]+)/g)) add(m[1], ref, 'prose')
  }
  for (const b of blogPosts) inline(b.content, `blog:${b.slug}.content`)
  for (const p of projects) inline(p.content, `project:${p.slug}.content`)
